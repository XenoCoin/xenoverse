...
#if defined(NDEBUG)
# error "XenoverseCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5e6aad5725eca16c9ef83fbffe0c5e6bb08aa962dae8a2793aa7ac5a602c6cd5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // XenoverseCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "XenoverseCoin Signed Message:\n";
...
{
    // XenoverseCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // XenoverseCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // XenoverseCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1508553915, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "The time 20 October 2017";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 5 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("048df612574bb0b4bf80dd87dc8c9e4eeaa0d2b12e488e797747e081cdd2050ac04bdab122b400f22b15f6da8e01c0f4cefb9737aa3ab411bce88f8bbe65ca0d34") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1508553915;
        ...
        assert(block.hashMerkleRoot == uint256("0x2e539d16c6d85c9994edceb008884c22110b56e45906f43f87e3748d126b6ef6"));
...
unsigned char pchMessageStart[4] = { 0x0e, 0x47, 0xa5, 0x41 }; // XenoverseCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("XenoverseCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("XenoverseCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static XenoverseCoinMiner(CWallet *pwallet)
{
    printf("XenoverseCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("xenoversecoin-miner");
    ...
    try { loop {
        ...
        printf("Running XenoverseCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("XenoverseCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&XenoverseCoinMiner, pwallet));
}
